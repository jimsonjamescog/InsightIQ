...
from insightiq.data.warehouse import build_warehouse, profile_warehouse, validate_warehouse
...
def main() -> int:
    ...
    if args.command in {"build", "reset"}:
        profile = build_warehouse(args.db, reset=args.command == "reset")
        print(json.dumps(profile.to_dict(), indent=2))
        return 0
    if not args.db.exists():
        raise SystemExit(f"Warehouse does not exist: {args.db}")
    if args.command == "validate":
        errors = validate_warehouse(args.db)
